public class ImprovedNeuron {
    ...
    public ImprovedNeuron(double LearningRate){
        learningRate = LearningRate;
        allInput = 0;
        outputLayerException = 0;
        inputLayerException = 0;
        allOutput = 0;
        inputLayer = new Neuron[1];
        layer1 = new Neuron[5];
        layer2 = new Neuron[7];
    ...
    public void Run(){
        ...
        inputLayer[0].Activate();

        for(int i=0;i<inputLayer.length;i++){
            for (int j=0;j<layer1.length;j++){
                layer1[j].input += inputLayer[i].output * weights[0][layer1.length*i+j];
            }
        }
        for (int i=0;i<layer1.length;i++){
            layer1[i].Activate();
    ...
    public void Train(){
        ...
        layer7[0].exception = this.outputLayerException * this.allOutput * (1 - this.allOutput);
        ...
        for (int i=0;i<inputLayer.length;i++){
            for (int j=0;j<layer1.length;j++){
                inputLayer[i].exception += weights[1][layer1.length*i+j] * layer1[j].exception * inputLayer[i].output * (1 - inputLayer[i].output);
            }
        }
        this.inputLayerException = inputLayer[0].exception;

        System.out.println(layer7[0].exception);

        // calculate and adding delta weights to actual weights;

        for (int i=0;i<inputLayer.length;i++){
            for (int j=0;j<layer1.length;j++){
                weights[0][layer1.length*i+j] += inputLayer[i].output * layer1[j].exception * learningRate;
            }
        }
    ...
    public static void main(String[]args){
        ImprovedNeuron improvedNeuron = new ImprovedNeuron(0.25);
        for (int i=0;i<100;i++){
            improvedNeuron.allInput=1;
            improvedNeuron.Run();
            improvedNeuron.outputLayerException = 1 - improvedNeuron.allOutput;
            System.out.println("a href:     "+improvedNeuron.allOutput);
            improvedNeuron.Train();
            improvedNeuron.Clear();

            improvedNeuron.allInput=0;
            improvedNeuron.Run();
            improvedNeuron.outputLayerException = 0 - improvedNeuron.allOutput;
            System.out.println("a href:     "+improvedNeuron.allOutput);
            improvedNeuron.Train();
            improvedNeuron.Clear();
